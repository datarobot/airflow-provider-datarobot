...
def test_compute_prediction_explanations_sensor__success(mocker):
    job_mock = mocker.Mock()
    job_mock.status = "COMPLETED"
    mocker.patch.object(dr.FeatureImpactJob, "get", return_value=job_mock)

    operator = ComputePredictionExplanationsSensor(
        task_id="check_compute_prediction_explanations_complete",
...
def test_compute_prediction_explanations_sensor__not_finished(mocker):
    job_mock = mocker.Mock()
...
def test_compute_prediction_explanations_sensor__raise_error(mocker):
    job_mock = mocker.Mock()
